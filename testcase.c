	...
	switch(choice){
		case 1: 
			...
			if(createPageFile (fileName) == RC_OK){
				...
				printf("\nCould not create file!!Please check\n");
			}
			break;
		...
		case 2:
			...
			if(openPageFile(fileName, &sm_FileHandle) == RC_OK){
				...
				printf("\nUnable to find the file...Please check the file name or if the file is already open\n");
			}
			break;

		case 3:
			if(closePageFile(&sm_FileHandle) == RC_FILE_NOT_FOUND){
				printf("\nNo file is opened..Please check!!!\n");
			}else{
		...
		case 5:
			printf("Enter the page number to be read:");
			scanf("%d", &pageNo);
			if(readBlock(pageNo, &sm_FileHandle, memPage) == RC_OK){
				printf("\nContents of the page\n%s\n", memPage);
				printf("\n");
			}else{
				printf("\nPlease check if file is open or page number is correct\n");
			}
			break;
		...
		case 15:
			...
			if(ensureCapacity (numberOfPages, &sm_FileHandle) == RC_FILE_NOT_FOUND){
				...
				printf ("\nThe total number pages : %d", numberOfPages);
			}
			break;
